-- +goose Up
-- Seed baseline physical nodes across Brahmaputra & Barak basins
...
-- Seed historical breach incident
...
-- +goose Down
DELETE FROM alert_logs WHERE id LIKE 'ALT-DEMO-%';
DELETE FROM macro_readings WHERE reach_id = 'REACH-MAJULI-01';
DELETE FROM node_telemetry WHERE node_id IN ('NODE-MAJULI-01', 'NODE-DIBRUGARH-02', 'NODE-SILCHAR-05');
DELETE FROM breach_records WHERE id IN ('BREACH-2022-SILCHAR', 'BREACH-2020-MAJULI');
DELETE FROM embankment_reaches WHERE id IN ('REACH-MAJULI-01', 'REACH-DIBRUGARH-02', 'REACH-TEZPUR-03', 'REACH-GUWAHATI-04', 'REACH-SILCHAR-05');
DELETE FROM embankment_nodes WHERE node_id IN ('NODE-MAJULI-01', 'NODE-DIBRUGARH-02', 'NODE-TEZPUR-03', 'NODE-GUWAHATI-04', 'NODE-SILCHAR-05');
